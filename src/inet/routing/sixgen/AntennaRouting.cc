...
Define_Module(AntennaRouting);

const int KIND_DELAYEDSEND = 100;

void AntennaRouting::initialize(int stage)
{
...
void AntennaRouting::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void AntennaRouting::handleMessageWhenUp(cMessage *msg)
{
...
void AntennaRouting::sendHelloMessagesIfNeeded()
{
...
void AntennaRouting::handleRREPACKTimer()
{
...
const Ptr<SNOOPHB> AntennaRouting::createHelloMessage()
{
...
INetfilter::IHook::Result AntennaRouting::ensureRouteForDatagram(Packet *datagram)
{
...
void AntennaRouting::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IRoute *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void AntennaRouting::scheduleExpungeRoutes()
{
...
void AntennaRouting::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
void AntennaRouting::sendSnooping(const Ptr<SNOOPHB>& snoop, unsigned int timeToLive){
...
void AntennaRouting::sendResp(const Ptr<RESPHB>& resp, const L3Address& destAddr, unsigned int timeToLive){
...
void AntennaRouting::sendCainMsg(const Ptr<CAINMSG>& cainmsg, unsigned int timeToLive, double delay){
    EV << "sending CAIN ";
...
void AntennaRouting::sendSprayMsg(const Ptr<CAINMSG>& cainMsg){
...
void AntennaRouting::sendHeartBeatpkg(const Ptr<HeartBeat>& hbpacket, const L3Address& destAddr, unsigned int timeToLive, double delay){
    ASSERT(timeToLive != 0);
...
const Ptr<SNOOPHB> AntennaRouting::createSnoopMsg(){
...
const Ptr<CHDEF> AntennaRouting::createChDefMsg(){
...
const Ptr<CAINMSG> AntennaRouting::createCainMsg(){
...
const Ptr<CAINMSG> AntennaRouting::createCainRespMsg(L3Address dest, L3Address originator){
    auto cainMsg = makeShared<CAINMSG>();
...
const Ptr<CAINMSG> AntennaRouting::createCainFwdMsg(L3Address dest){
    auto cainMsg = makeShared<CAINMSG>();
    cainMsg->setHops(0);
    cainMsg->setPacketType(usingIpv6 ? CAINFWD_IPv6 : CAINFWD);
    cainMsg->setChunkLength(usingIpv6 ? B(48) : B(24));
...
const Ptr<CAINMSG> AntennaRouting::createCainReqMsg(L3Address dest){
    auto cainMsg = makeShared<CAINMSG>();
    cainMsg->setHops(0);
    cainMsg->setPacketType(usingIpv6 ? CAINREQ_IPv6 : CAINREQ);
    cainMsg->setChunkLength(usingIpv6 ? B(48) : B(24));
...
const Ptr<CAINMSG> AntennaRouting::createCainHopMsg(L3Address dest){
    auto cainMsg = makeShared<CAINMSG>();
    cainMsg->setHops(0);
...
const Ptr<DRONEMSG> AntennaRouting::createDroneMsg(){
...
const Ptr<ANTENNA> AntennaRouting::createAntennaMsg(){
...
const Ptr<FLWEIGHT> AntennaRouting::createWeightMsg(std::vector<float> *weights){
...
const Ptr<CAINMSG> AntennaRouting::createCainErrMsg(L3Address dest, L3Address originator){
    auto cainMsg = makeShared<CAINMSG>();
...
const Ptr<CAINMSG> AntennaRouting::createCainAck(L3Address dest,heartBeatType cainMsgType,int seqNum){
    auto cainMsg = makeShared<CAINMSG>();
...
const Ptr<CAINMSG> AntennaRouting::createAckTimer(heartBeatType cainMsgType,int seqNum){
    auto cainMsg = makeShared<CAINMSG>();
    cainMsg->setPacketType(usingIpv6 ? CAINACK_IPv6: CAINACK);
    cainMsg->setChunkLength(usingIpv6 ? B(48) : B(24));
...
const Ptr<RESPHB> AntennaRouting::createRespHBMsg(L3Address dest){
...
const Ptr<CAINMSG> AntennaRouting::createCainRREQ(){
    auto cainMsg = makeShared<CAINMSG>();
    EV << "Node " << getSelfIPAddress() << " is creating a ";
...
const Ptr<CAINMSG> AntennaRouting::createSCMSG(){
    auto cainMsg = makeShared<CAINMSG>();
    cainMsg->setHops(0);
    EV << "Node " << getSelfIPAddress() << " is creating a ";
...
const Ptr<CAINMSG> AntennaRouting::createCainRREP(L3Address nextHop,L3Address rreqDest,L3Address cainSource
        ,int battery){
    sentCainRREPMsg++;
...
const Ptr<CAINMSG> AntennaRouting::createLARMSG(){
    auto cainMsg = makeShared<CAINMSG>();
    cainMsg->setHops(0);
    EV << "Node " << getSelfIPAddress() << " is creating a ";
...
const Ptr<CAINMSG> AntennaRouting::createSPRAYMSG(int numNodes){
    auto cainMsg = makeShared<CAINMSG>();
    cainMsg->setHops(0);
    EV << "Node " << getSelfIPAddress() << " is creating a ";
...
const Ptr<CAINMSG> AntennaRouting::createBRAPMSG(){
    auto cainMsg = makeShared<CAINMSG>();
    cainMsg->setHops(0);
    EV << "Node " << getSelfIPAddress() << " is creating a ";
...
L3Address AntennaRouting::findHopRL(){
...
bool AntennaRouting::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

INetfilter::IHook::Result AntennaRouting::datagramForwardHook(Packet *datagram)
{
...
void AntennaRouting::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
...
void AntennaRouting::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> AntennaRouting::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
bool AntennaRouting::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        HeartBeatRouteData *routeData = check_and_cast<HeartBeatRouteData *>(route->getProtocolData());
...
void AntennaRouting::processPacket(Packet *packet)
{
...
void AntennaRouting::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void AntennaRouting::socketClosed(UdpSocket *socket)
{
    if (operationalState == State::STOPPING_OPERATION)
        startActiveOperationExtraTimeOrFinish(par("stopOperationExtraTime"));
}

void AntennaRouting::handleStartOperation(LifecycleOperation *operation)
{
...
void AntennaRouting::handleSnooping(const Ptr<SNOOPHB>& snoop, const L3Address& sourceAddr){
...
void AntennaRouting::handleLeachMsg(const Ptr<CHDEF>& leach){
...
void AntennaRouting::handleResp(const Ptr<RESPHB>& resp){
...
void AntennaRouting::handleCainFWD(const Ptr<CAINMSG>& cainmsg){
...
void AntennaRouting::handleCainREQ(const Ptr<CAINMSG>& cainmsg){
    EV << "Destined to: " << cainmsg->getDestAddr() << ", initiated by: " << cainmsg->getOriginatorAddr() <<
                ", sent by: " << cainmsg->getSourceAddr() << endl;
    EV << "Self ipAddr: " << getSelfIPAddress() << endl;
...
void AntennaRouting::handleAntennaMsg(const Ptr<ANTENNA>& antennaMsg){
...
void AntennaRouting::handleDroneMsg(const Ptr<DRONEMSG>& droneMsg){
...
void AntennaRouting::handleWeightMsg(const Ptr<FLWEIGHT>& weightMsg){
...
void AntennaRouting::handleCainRESP(const Ptr<CAINMSG>& cainmsg){
    EV << "Destined to: " << cainmsg->getDestAddr() << ", initiated by: " << cainmsg->getOriginatorAddr() <<
                ", sent by: " << cainmsg->getSourceAddr() << endl;
    EV << "Self ipAddr: " << getSelfIPAddress() << endl;
...
void AntennaRouting::handleCainHOP(const Ptr<CAINMSG>& cainmsg){
    EV << "Destined to: " << cainmsg->getDestAddr() << ", initiated by: " << cainmsg->getOriginatorAddr() <<
                ", sent by: " << cainmsg->getSourceAddr() << endl;
    EV << "Self ipAddr: " << getSelfIPAddress() << endl;
...
void AntennaRouting::handleCainERR(const Ptr<CAINMSG>& cainmsg){
    EV << "Destined to: " << cainmsg->getDestAddr() << ", initiated by: " << cainmsg->getOriginatorAddr() <<
            ", sent by: " << cainmsg->getSourceAddr() << endl;
    EV << "Self ipAddr: " << getSelfIPAddress() << endl;
...
void AntennaRouting::handleCainACK(const Ptr<CAINMSG>& cainmsg){
    EV << "Destined to: " << cainmsg->getDestAddr() << ", initiated by: " << cainmsg->getOriginatorAddr() <<
                ", sent by: " << cainmsg->getSourceAddr() << endl;
    EV << "Self ipAddr: " << getSelfIPAddress() << endl;
...
void AntennaRouting::handleFwdTimer(){
    if(strcmp(this->getParentModule()->getName(),"antenna")){
...
void AntennaRouting::handleCainRREQ(const Ptr<CAINMSG>& cainmsg){
    EV << "Destined to: " << cainmsg->getDestAddr() << ", initiated by: " << cainmsg->getOriginatorAddr() <<
                ", sent by: " << cainmsg->getSourceAddr() << endl;
    EV << "Self ipAddr: " << getSelfIPAddress() << endl;
    if(strcmp(this->getParentModule()->getName(),"antenna")){
...
void AntennaRouting::handleCainRREP(const Ptr<CAINMSG>& cainmsg){
    EV << "Destined to: " << cainmsg->getDestAddr() << ", initiated by: " << cainmsg->getOriginatorAddr() <<
                ", sent by: " << cainmsg->getSourceAddr() << endl;
    EV << "Self ipAddr: " << getSelfIPAddress() << endl;
    if(strcmp(this->getParentModule()->getName(),"antenna")){
...
void AntennaRouting::handleSCMSG(const Ptr<CAINMSG>& cainmsg){
...
void AntennaRouting::handleCainLAR(const Ptr<CAINMSG>& cainmsg){
    EV << "Destined to: " << cainmsg->getDestAddr() << ", initiated by: " << cainmsg->getOriginatorAddr() <<
                ", sent by: " << cainmsg->getSourceAddr() << endl;
    EV << "Self ipAddr: " << getSelfIPAddress() << endl;
    if(recFwdMessages->find(cainmsg->getOriginatorAddr()) == recFwdMessages->end()){
...
void AntennaRouting::handleCainSPR(const Ptr<CAINMSG>& cainmsg){
    EV << "Destined to: " << cainmsg->getDestAddr() << ", initiated by: " << cainmsg->getOriginatorAddr() <<
                ", sent by: " << cainmsg->getSourceAddr() << endl;
    EV << "Self ipAddr: " << getSelfIPAddress() << endl;
    if(recFwdMessages->find(cainmsg->getOriginatorAddr()) == recFwdMessages->end()){
...
void AntennaRouting::handleCainBRAP(const Ptr<CAINMSG>& cainmsg){
    EV << "Destined to: " << cainmsg->getDestAddr() << ", initiated by: " << cainmsg->getOriginatorAddr() <<
                    ", sent by: " << cainmsg->getSourceAddr() << endl;
    EV << "Self ipAddr: " << getSelfIPAddress() << endl;
...
void AntennaRouting::handleCainReply(const Ptr<CAINMSG>& cainmsg){
    if(strcmp(this->getParentModule()->getName(),"antenna")){
...
void AntennaRouting::handleCainHopCount(const Ptr<CAINMSG>& cainmsg){
    hopMap->operator [](cainmsg->getOriginatorAddr())=cainmsg->getHops()+1;
}



void AntennaRouting::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void AntennaRouting::recLeachMsg(L3Address neighAddr, const Ptr<SNOOPHB>& snoop){
...
void AntennaRouting::calcPheromone(L3Address neighAddr, const Ptr<CAINMSG>& cainmsg){
...
/*void AntennaRouting::recLeachRespMsg(const Ptr<RESPHB>& respMsg){
...
void AntennaRouting::chDecision(){
...
void AntennaRouting::resetLeachCalculation(){
...
void AntennaRouting::updateChCandidate(L3Address candidate, int batteryPercent, Coord senderCoord){
...
IRoute *AntennaRouting::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void AntennaRouting::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void AntennaRouting::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void AntennaRouting::clearState()
{
...
L3Address AntennaRouting::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void AntennaRouting::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void AntennaRouting::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void AntennaRouting::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void AntennaRouting::cancelRouteDiscovery(const L3Address& destAddr)
{
...
double AntennaRouting::cainBackoff(){
    int p;
    if(currBackoff > 4)
        p = pow(2,4);
    else
...
void AntennaRouting::updateConnectedDevs(){
//    connectedDevs->clear();
...
void AntennaRouting::updateBestHop(L3Address srcAddr){
...
void AntennaRouting::updateLarBestHop(L3Address srcAddr, Coord senderCoord){
    Coord thisCoord = baseMobility->getCurrentPosition();
...
int AntennaRouting::get_coverage_state(L3Address cain_dest){
    if(rl_type == "Euclidean1" || rl_type == "Euclidean2"  ||
                rl_type == "Euclidean3"){
...
void AntennaRouting::calculate_coverage_reward(int state,bool decision,L3Address cain_dest){
    double percent;
    if(rl_type == "Euclidean1" || rl_type == "Euclidean2"  ||
                rl_type == "Euclidean3"){
        double dist = distMap->at(cain_dest);
...
void AntennaRouting::updatePowerThreshold(int state){
...
bool AntennaRouting::sendMessageML(int state){
...
void AntennaRouting::calculate_q_matrix(){
...
void AntennaRouting::calculate_drone_q_matrix(){
    float discount_rate = 0.95;
    int n_interations = 100;
...
void AntennaRouting::create_reward_matrix(){
    vector<float>* a0;
    vector<float>* a1;
    if(rl_type == "Euclidean1" || rl_type == "Hop1"){
        //higher distance/hop
